...
for (var i = 0; i<floor(ivDivided);i++){//for each full row
	...
	for (var ii=0; ii<objectsPerRow;ii++){
		...
		if collision_rectangle(xx-spr.sprite_width/2,yy-spr.sprite_height/2,xx+spr.sprite_width/2,yy+spr.sprite_height/2,obj_cursor,1,1){
		//idk it's clicked	
		show_debug_message("clicked item "+ string(i)+" object: "+ string(ds_list_find_value(global.inventory,i)))
		}
	}
